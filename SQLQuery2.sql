create database ram;
use ram;
create table employee(id int primary key identity,name varchar(50),email varchar(50),mobile varchar(50),gender char(50));
alter table employee add address varchar(200);
select * from employee;
insert into employee(id ,name,email,mobile,gender,address) values(101,'Ajay','abc@gmail.com',535465,'male','loni');
...
1.
Select *from employees;
CREATE PROCEDURE spGetEmployees_help
AS
BEGIN
   select first_name from employees
END

 execute spGetEmployees_help
 
2.create procedure spGethireDate
as 
begin
   select hire_date ,first_name from employees;
END

execute spGethireDate

3.create procedure spGetsEmp_name
as
begin
select first_name+','+cast(job_id as varchar(20)) as employee_and_title from employees;
end

execute spGetsEmp_name

4.create procedure gertHireDate
as 
begin
select hire_date,first_name+' '+last_name as name, department_id from employees
end
 execute gertHireDate
 
5.create procedure allDetail
as
begin
select first_name+' '+last_name,department_id,salary,employee_id from employees
end
execute allDetail 

6.alter procedure spGetEmployees_help
@salary int=0
as  
begin 
select first_name+' '+last_name as name,salary from employees  where salary>@salary;
end
execute spGetEmployees_help 2000

7.create procedure alldateDetail
as
begin
select first_name+' '+last_name as Name,hire_date as Start_Date from employees
end
execute alldateDetail 

8.create procedure allHiredateDetail
as
begin
select first_name+' '+last_name as Name,hire_date as Start_Date from employees order by hire_date;
end
execute allHiredateDetail 

9.
create procedure allsalarydateDetail
as
begin
select first_name+' '+last_name as Name,salary  as salary from employees order by salary ASC;
end
execute allsalarydateDetail 

10.alter procedure spGetEmployees_help
@commission int=0
as
begin
select first_name+' '+last_name as Name,department_id  as Dept_id from employees where commission<@commission order by salary DESC;
end
execute spGetEmployees_help 0

11.
alter procedure spGetEmployees_help
AS
begin 
select employees.last_name,jobs.job_title as jobTitle from employees inner join jobs on employees.job_id=jobs.job_id;
end
execute spGetEmployees_help

12.alter procedure spGetEmployees_help
@salary int=0,
@job_title varchar(100)
as
begin
select employees.last_name,employees.salary,jobs.job_title from employees inner join jobs on employees.job_id=jobs.job_id where  job_title=@job_title;
end
execute spGetEmployees_help @job_title='Sales Representative'